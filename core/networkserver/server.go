...
func (s *networkServerRPC) ValidateContext(ctx context.Context) error {
	...
	if !ok {
		return core.NewErrInternal("Could not get metadata from context")
	}
	var id, token string
	...
	if id == "" {
		return core.NewErrInvalidArgument("Metadata", "id missing")
	}
	if tokens, ok := md["token"]; ok && len(tokens) == 1 {
		token = tokens[0]
	}
	if token == "" {
		return core.NewErrInvalidArgument("Metadata", "token missing")
	}
	...
	if claims.Subject != id {
		return core.NewErrInvalidArgument("Metadata", "token was issued for a different component id")
	}
	return nil
}
